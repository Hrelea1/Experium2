...
async function sendEmail(to: string, subject: string, htmlContent: string): Promise<void> {
  ...
  if (!apiKey) {
    ...
    console.log('  BREVO_API_KEY is missing — email was NOT sent!');
    console.log('------------------------------------------------------\n');
    return;
  }
...
export async function sendOtpEmail(email: string, otp: string, name?: string): Promise<void> {
  await sendEmail(email, 'Codul tău de verificare Experium', `
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
      ...
      <p style="color: #555; line-height: 1.6;">Pentru a continua procesul pe platforma noastră, te rugăm să folosești codul de securitate de mai jos. Acest cod confirmă că tu ești cel care a inițiat acțiunea.</p>
      <div style="background: #fff5f2; border: 2px dashed #F06236; border-radius: 12px; padding: 24px; text-align: center; margin: 24px 0;">
        <span style="font-size: 40px; font-weight: bold; letter-spacing: 8px; color: #F06236;">${otp}</span>
      </div>
...
export async function sendPasswordResetEmail(email: string, otp: string, name?: string): Promise<void> {
  await sendEmail(email, ' Resetare parolă Experium', `
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #eee;">
      <div style="background: linear-gradient(135deg, #F06236 0%, #F59E0B 100%); padding: 32px; text-align: center;">
        <h1 style="color: #fff; margin: 0; font-size: 24px;"> Resetare Parolă</h1>
        <p style="color: rgba(255,255,255,0.9); margin: 8px 0 0;">Experium</p>
      </div>
      <div style="padding: 32px;">
        <p style="color: #333; font-size: 16px;">Bună${name ? ` <strong>${name}</strong>` : ''},</p>
        <p style="color: #555; line-height: 1.6;">Am primit o cerere de resetare a parolei pentru contul tău Experium. Pentru a-ți proteja contul, am generat un cod unic de securitate. Te rugăm să-l introduci în fereastra aplicației pentru a putea alege o nouă parolă:</p>
        <div style="background: #fff5f2; border-radius: 12px; padding: 28px; text-align: center; margin: 24px 0; border: 2px dashed #F06236;">
          <p style="margin: 0 0 8px; color: #888; font-size: 14px;">Codul tău de resetare</p>
          <span style="font-size: 44px; font-weight: bold; letter-spacing: 10px; color: #F06236;">${otp}</span>
        </div>
        <p style="color: #e67e22; font-size: 14px;">⏱ Codul este valabil <strong>10 minute</strong>.</p>
        <p style="color: #888; font-size: 13px; margin-top: 24px; border-top: 1px solid #eee; padding-top: 16px;">Dacă nu ai solicitat resetarea parolei, te rugăm să ignori acest mesaj. Contul tău este în siguranță și nicio modificare nu va fi făcută.</p>
      </div>
    </div>
  `);
}
  ...
  await sendEmail(params.email, `Rezervare confirmată — ${params.experienceTitle}`, `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 480px; margin: 0 auto; background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #eee; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
      ...
      <div style="padding: 32px;">
        ...
        <p style="color: #555; line-height: 1.6;">Îți mulțumim că ai ales Experium! Rezervarea ta a fost înregistrată cu succes în sistemul nostru. Mai jos găsești toate detaliile aventurii tale:</p>
        <h3 style="color: #1a1a2e; margin: 16px 0 8px; font-size: 18px; border-left: 4px solid #F06236; padding-left: 12px;">${params.experienceTitle}</h3>
        <div style="background: #f8f8f8; border-radius: 12px; padding: 20px; margin: 24px 0; border: 1px solid #eaeaea;">
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr><td style="padding: 12px 0 4px; color: #666; font-size: 14px;"> Total plătit:</td><td style="padding: 12px 0 4px; text-align: right; color: #F06236; font-weight: bold; font-size: 16px;">${params.totalPrice} RON</td></tr>
          </table>
        </div>
        <p style="color: #555; line-height: 1.6; margin: 24px 0;">Îți recomandăm să ajungi la locație cu 10-15 minute înainte de ora stabilită.</p>
...
export async function sendProviderBookingNotification(params: {
  ...
  await sendEmail(params.providerEmail, `Rezervare nouă — ${params.experienceTitle}`, `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 480px; margin: 0 auto; background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #eee; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
      <div style="background: linear-gradient(135deg, #263241 0%, #1a1a2e 100%); padding: 32px; text-align: center;">
      ...
      <div style="padding: 32px;">
        ...
        <p style="color: #555; line-height: 1.6;">O nouă rezervare a fost confirmată și plătită pe platformă. Un client abia așteaptă să participe la experiența oferită de tine:</p>
        <h3 style="color: #1a1a2e; margin: 16px 0 8px; font-size: 18px; border-left: 4px solid #F59E0B; padding-left: 12px;">${params.experienceTitle}</h3>
        <div style="background: #f8f8f8; border-radius: 12px; padding: 20px; margin: 24px 0; border: 1px solid #eaeaea;">
          <table style="width: 100%; border-collapse: collapse;">
        ...
        <div style="text-align: center; margin: 24px 0;">
          <a href="https://hrelea1.github.io/Experium2/provider/dashboard" style="background: #263241; color: white; padding: 12px 28px; border-radius: 8px; text-decoration: none; font-weight: 500; font-size: 14px; display: inline-block;"> Accesează Dashboard-ul</a>
        </div>
        <div style="margin-top: 32px; border-top: 1px solid #eee; padding-top: 16px;">
          <p style="color: #888; font-size: 12px; margin: 0;">ID Rezervare: <span style="font-family: monospace; background: #f0f0f0; padding: 2px 6px; border-radius: 4px;">${params.bookingId}</span></p>
        </div>
  ...
  await sendEmail(params.email, `Rezervare anulată — ${params.experienceTitle}`, `
    <div style="font-family: Arial, sans-serif; max-width: 540px; margin: 0 auto;">
      ...
      ${params.refundEligible
        ...
        : '<p style="color: #e67e22;"> Anulare târzie — politica noastră nu permite ramburs pentru anulări sub 48h înainte de experiență.</p>'
      }
  ...
  await sendEmail(params.providerEmail, `Cerere disponibilitate — ${params.experienceTitle}`, `
    <div style="font-family: Arial, sans-serif; max-width: 540px; margin: 0 auto;">
      ...
      <div style="text-align: center; margin: 32px 0;">
        <a href="${params.confirmUrl}" style="background:#F06236; color:white; padding:14px 32px; border-radius:8px; text-decoration:none; margin-right:16px; font-weight:bold;"> Confirmă</a>
        <a href="${params.declineUrl}" style="background:#1a1a2e; color:white; padding:14px 32px; border-radius:8px; text-decoration:none; font-weight:bold;"> Refuză</a>
      </div>
...
export async function sendProviderVoucherNotification(params: {
  ...
  await sendEmail(params.providerEmail, `Voucher nou achiziționat — ${params.experienceTitle}`, `
    <div style="font-family: Arial, sans-serif; max-width: 540px; margin: 0 auto;">
      <h2 style="color: #1a1a2e;"> Un nou voucher a fost achiziționat!</h2>
      <p>Bună ${params.providerName},</p>
...
export async function sendNewsletterWelcome(email: string): Promise<void> {
  await sendEmail(email, ' Bun venit în comunitatea Experium!', `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: 0 auto; background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #eee;">
      <div style="background: linear-gradient(135deg, #263241 0%, #1a1a2e 100%); padding: 40px 32px; text-align: center;">
      ...
      <div style="padding: 36px 32px;">
        ...
        <div style="text-align: center; margin: 32px 0;">
        <a href="https://hrelea1.github.io/Experium2" style="background: linear-gradient(135deg, #F06236 0%, #F59E0B 100%); color: white; padding: 14px 36px; border-radius: 10px; text-decoration: none; font-weight: bold; font-size: 15px; display: inline-block;"> Explorează experiențele</a>
        </div>
...
export async function sendBookingDeclinedAdminAlert(params: {
  ...
  await sendEmail(params.adminEmail, ` Rezervare refuzată de furnizor — ${params.experienceTitle}`, `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: 0 auto; background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #eee;">
      <div style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); padding: 28px 32px; text-align: center;">
        <h1 style="color: #fff; margin: 0; font-size: 22px;"> Rezervare Refuzată de Furnizor</h1>
